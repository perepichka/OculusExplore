...
namespace Streetview
{
    public class StreetViewSticher : MonoBehaviour
    {
        ...
        {       
            ...
            // If the neighbors have been loaded
        ...
        public bool AttemptAddingPoint(Vector2 point, Vector2 direction, bool goToAfter, bool asynch)
        {
            ...
            {
                bool result = AddPoint(point, goToAfter, asynch);
                if (result)
                {
                    return true;
                }
        ...
        // Adds the coordinate point. True if successful or stich exists already, false otherwise
        public bool AddPoint(Vector2 point, bool goToAfter, bool asynch)
        {
        ...
        // Combines texture into another texture, yielding in between
        //IEnumerator CombineTextureListAsync(List<List<Texture2D>> images, int imageSize)
        //{
        //    // Loads texture from 
        //    Texture2D t = new Texture2D(imageSize * images[0].Count, imageSize * images.Count);
        //    t.wrapMode = TextureWrapMode.Clamp;
        ...
        public bool SetCurrentStich(string coordinates)
        {
            ...
            // The attempt to get the value failed
            return false;
        }

        public void AddNeighborStichs()
        {
            Stich s;
            Vector2 pos;
            ...
            if (Stiches.TryGetValue(CurrentStichCoordinates, out s))
            {
                // Checks if the neighbors have already been loaded
                if (s.NeighborsLoaded)
                {
                    return;
                }
                pos = new Vector2(s.Lat, s.Lng);
            }
            else
            {
                return;
            ...
            foreach (Vector2 dir in StreetViewController.Directions)
            {
                StartCoroutine(StartAttemptAddingStichJob(pos, dir));
            }
            s.NeighborsLoaded = true;
        }

        //
        // CoRoutine and passive cache filling
        //


        // Starts a coroutine to add a stich
        IEnumerator StartAttemptAddingStichJob(Vector2 point, Vector2 dir)
        {
            bool res = AttemptAddingPoint(point + dir * CoordinateIncrement, dir, false, true);
